...
def encode_video(videoName, secretData):
    if os.path.exists("./stegoVideo.avi"):
        os.remove("./stegoVideo.avi")

    framecount = extract_frames(videoName)
            ...
            if dataindex < datalen:
                for row in image:
                    ...
                    for pixel in row:
                        ...
                        r, g, b = convert_to_bin(pixel) #Convert RGB values to binary format
                        if dataindex < datalen: #Modify the red pixel bits depending on bitRange only if there is still data to store
                            if (datalen - dataindex) < bitRange: #If data left to store is less than the bitRange, left shift the data accordingly then store so that it can be decoded correctly later
                                pixel[0] = int(r[:-bitRange] + convert_to_bin(int(binarysecretdata[dataindex:(dataindex + (datalen - dataindex))]) << (bitRange-(datalen-dataindex))), 2)
                        ...
                        if dataindex < datalen: #Modify the green pixel bits depending on bitRange only if there is still data to store
                            if (datalen - dataindex) < bitRange: #If data left to store is less than the bitRange, left shift the data accordingly then store so that it can be decoded correctly later
                                pixel[1] = int(g[:-bitRange] + convert_to_bin(int(binarysecretdata[dataindex:(dataindex + (datalen - dataindex))]) << (bitRange-(datalen-dataindex))), 2)
                                dataindex += (datalen - dataindex)
                            else:
                        ...
                        if dataindex < datalen: #Modify the blue pixel bits depending on bitRange only if there is still data to store
                            if (datalen - dataindex) < bitRange: #If data left to store is less than the bitRange, left shift the data accordingly then store so that it can be decoded correctly later
                                pixel[2] = int(b[:-bitRange] + convert_to_bin(int(binarysecretdata[dataindex:(dataindex + (datalen - dataindex))]) << (bitRange-(datalen-dataindex))), 2)
                                dataindex += (datalen - dataindex)
                            else:
...
while True:
    ...
    if userInput == "1":
        ...
        encode_video(videoName, textToHide)
    ...
    #Decode
        ...
        decodedText = decode_video(videoName)
